...
import unittest
from python_utils.common.cli import LinuxCLI
...
class CLITest(unittest.TestCase):
    ...
    def test_pipes(self):
        ...
        try:
            ret6 = cli.cmd_pipe([['ls'], ['tee', 'test-file6']],
                                blocking=False)
    ...
    def test_host_file_replacement(self):
        ...
        if cli.exists('/etc/hosts.backup'):
            self.fail(
                "Backup file present, please cleanup to "
            ...
            try:
                ...
                self.assertTrue(cli.grep_file('/etc/hosts',
                                              '1.1.1.1 foo'))
                self.assertTrue(cli.grep_file('/etc/hosts',
                                              '2.2.2.2 bar'))
                self.assertTrue(cli.grep_file('/etc/hosts',
                                              '64.64.64.64 dontchange'))
                self.assertTrue(cli.grep_file('/etc/hosts',
                                              '3.3.3.3 baz'))
                ...
                self.assertEqual(
                    1,
                    len(cli.cmd(
                        'grep "13.13.13.13 baz" /etc/hosts')
                        .stdout
                        .splitlines(False)))
                self.assertFalse(cli.grep_file('/etc/hosts',
                                               '12.12.12.12 baz'))
...
test_utils.run_unit_test(CLITest)
